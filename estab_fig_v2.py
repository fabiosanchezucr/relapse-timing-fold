...
d=json.load(open('gres.json'))
E=d["establishment"]; seeds=[0.03,0.07,0.11,0.15]
...
for k,c,lab in [("early",ACC,"early relapse (declining hazard)"),
    ...
    p=[E[f"{k}|{s}"][0] for s in seeds]; lo=[max(E[f"{k}|{s}"][1],0) for s in seeds]
    hi=[min(E[f"{k}|{s}"][2],1) for s in seeds]
    ax.plot(seeds,p,"o-",color=c,lw=1.9,ms=6,label=lab); ax.fill_between(seeds,lo,hi,color=c,alpha=0.15)
...
ax.set_title("A  Establishment at $\\mathcal{R}_0=0.95$ (corrected mesh)",loc="left",fontweight="bold",color=INK)
# panel B: robustness of the const value across mesh/threshold/system size
cats=["mesh\nN=20","mesh\nN=40","mesh\nN=60","thr\n0.12","thr\n0.17","thr\n0.25","$N_0$\n750","$N_0$\n1500","$N_0$\n3000"]
vals=[d["mesh"]["N20"][1],d["mesh"]["N40"][1],d["mesh"]["N60"][1],
      d["threshold"]["0.12"][1],d["threshold"]["0.17"][1],d["threshold"]["0.25"][1],
      d["finitesize"]["750"][1],d["finitesize"]["1500"][1],d["finitesize"]["3000"][1]]
n=60; err=[1.96*np.sqrt(max(v*(1-v),1e-4)/n) for v in vals]
...
fig.suptitle("Corrected stochastic results: ordering early $\\gg$ constant $\\gg$ late is discretization-independent",
             x=0.075,ha="left",fontweight="bold",fontsize=11.5,color=INK)
fig.savefig("figs/relapse_establishment_v2.pdf",facecolor="white")
fig.savefig("estab_v2.png",dpi=165,facecolor="white")
print("saved. establishment(const):",[round(E[f'const|{s}'][0],2) for s in seeds])
